 ...
 * </p>
 * <p>
 * In einem Objekt der Klasse BinarySearchTree werden beliebig viele Objekte in
 * einem Binaerbaum (binaerer Suchbaum) entsprechend einer Ordnungsrelation
 * verwaltet. Ein Objekt der Klasse stellt entweder einen leeren Baum dar oder
 * verwaltet ein Inhaltsobjekt sowie einen linken und einen rechten Teilbaum,
 * die ebenfalls Objekte der Klasse BinarySearchTree sind. Dabei gilt: Die
 * Inhaltsobjekte sind Objekte einer Unterklasse von Item, in der durch
 * &uuml;berschreiben der drei Vergleichsmethoden isLess, isEqual, isGreater (s.
 * Item) eine eindeutige Ordnungsrelation festgelegt sein muss. Alle Objekte im
 * linken Teilbaum sind kleiner als das Inhaltsobjekt des Binaerbaumes. Alle
 * Objekte im rechten Teilbaum sind groesser als das Inhaltsobjekt des
 * Binaerbaumes. Diese Bedingung gilt auch in beiden Teilbaeumen.
 * 
 * </p>
 * 
 * <p>
...
public class BinarySearchTree {
	...
	/**
	 * Falls ein bezueglich der verwendeten Vergleichsmethode isEqual mit pItem
	 * uebereinstimmendes Objekt im geordneten Baum enthalten ist, passiert
	 * nichts. Andernfalls wird das Objekt pItem entsprechend der vorgegebenen
	 * Ordnungsrelation in den Baum eingeordnet. Falls der Parameter null ist,
	 * aendert sich nichts.
	 * 
	 * @param pItem
	...
	public void insert(Item pItem) {
		if (pItem != null) {
			if (bintree.isEmpty()) // wenn der Suchbaum leer ist, dann wird der
									// Suchbaum mit dem Inhalt pItem gefuellt
				...
				} else if (pItem.isGreater(lItem)) { // rechts Einfuegen; bei
														// Gleichheit wird nicht
														// noch einmal
														// eingefuegt
	...
	/**
	 * Falls ein bezueglich der verwendeten Vergleichsmethode isEqual mit pItem
	 * uebereinstimmendes Objekt im binaeren Suchbaum enthalten ist, liefert die
	 * Anfrage dieses, ansonsten wird null zurueckgegeben. Falls der Parameter
	 * null ist, wird null zurueckgegeben.
	 * 
	 * @param pItem
	 *            zu suchendes Objekt
	...
	/**
	 * Falls ein bezueglich der verwendeten Vergleichsmethode isEqual mit pItem
	 * uebereinstimmendes Objekt im binaeren Suchbaum enthalten ist, wird dieses
	 * entfernt. Falls der Parameter null ist, aendert sich nichts.
	 * 
	 * @param pItem
				...
				if (bintree.getRightTree().isEmpty()
					...
					if (bintree.getRightTree().isEmpty()) {
						...
						if (bintree.getLeftTree().isEmpty()) {
							...
							while (!lGroessterLinkerKnoten.getRightTree()
									.isEmpty())
								lGroessterLinkerKnoten = lGroessterLinkerKnoten
										.getRightTree();
							bintree.setObject(lGroessterLinkerKnoten
									.getObject());
							BinarySearchTree lLinkerBaum = this.getLeftTree();
							this.getLeftTree().remove(
									(Item) lGroessterLinkerKnoten.getObject());
						}
					}
				}
			} else {
				if (lWurzelInhalt.isLess(pItem)) { // rekursiv loeschen im
													// rechten Teilbaum
	...
	/**
	 * Diese Anfrage liefert den linken Teilbaum des binaeren Suchbaumes. Der
	 * binaere Suchbaum aendert sich nicht. Wenn er leer ist, wird null
	 * zurueckgegeben.
	 * 
	...
	/**
	 * Diese Anfrage liefert den rechten Teilbaum des binaeren Suchbaumes. Der
	 * binaere Suchbaum aendert sich nicht. Wenn er leer ist, wird null
	 * zurueckgegeben.
	 * 
